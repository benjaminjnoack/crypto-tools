...
describe("coinbase signing", () => {
  ...
  it("loads keys from credentials and signs with expected payload/header", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(dateUtc({ year: 2026, month: 3, day: 1, hour: 12, minute: 34, second: 56 }));
    const nowSec = Math.floor(dateUtc({ year: 2026, month: 3, day: 1, hour: 12, minute: 34, second: 56 }).getTime() / 1000);
