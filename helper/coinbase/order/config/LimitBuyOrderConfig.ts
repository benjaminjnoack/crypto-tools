import OrderConfig, { OrderConfigJSONSchema } from './OrderConfig';
import { POSITION_KEYS } from '@core/dictionary';
import { safeNumber } from '@core/validation.js';
import { z } from 'zod';

export const LimitBuyOrderConfigJSONSchema = OrderConfigJSONSchema.extend({
  [POSITION_KEYS.LIMIT_PRICE]: z.string(),
});

export type LimitBuyOrderConfigJSON = z.infer<typeof LimitBuyOrderConfigJSONSchema>;

export default class LimitBuyOrderConfig extends OrderConfig {
  private readonly _limit_price: string;
  constructor(
    uuid: string,
    base_size: string,
    limit_price: string,
    order_id: string | null = null,
  ) {
    super(uuid, base_size, order_id);
    safeNumber(limit_price, 'LimitBuyOrderConfig => limit_price', true);
    this._limit_price = limit_price;
  }

  toJSON() {
    return {
      ...super.toJSON(),
      [POSITION_KEYS.LIMIT_PRICE]: this._limit_price,
    };
  }

  static fromJSON(json: LimitBuyOrderConfigJSON) {
    return new LimitBuyOrderConfig(
      json[POSITION_KEYS.UUID],
      json[POSITION_KEYS.BASE_SIZE],
      json[POSITION_KEYS.LIMIT_PRICE],
      json[POSITION_KEYS.ORDER_ID],
    );
  }
}
